...
const MAPS_ICON_SVG = `<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 5644.173 5644.173">
	<defs>
...
const decorateMapsLink = (link) => {
  if (!link || link.dataset.mapsDecorated) {
    return;
  }
  link.dataset.mapsDecorated = "true";
  link.classList.add("maps-link");
  const iconWrap = document.createElement("span");
  iconWrap.className = "maps-icon";
  iconWrap.innerHTML = MAPS_ICON_SVG;
  link.prepend(iconWrap);
};

window.decorateMapsLink = decorateMapsLink;
...
document.addEventListener("click", (event) => {
  ...
  if (!navigator.onLine) {
    ...
    showToast("You are offline. Connect to open external links.");
  }
});
...
updateOnlineStatus();
setActiveNav();
document.querySelectorAll("a[data-maps=\"true\"]").forEach((link) => {
  decorateMapsLink(link);
});
